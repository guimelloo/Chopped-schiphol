...
<template>
    <Head :title="t('flightList.title')" />
    <SchipholLayout>
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900">{{ t('flightList.title') }}</h1>
                <p class="mt-2 text-gray-600">{{ t('flightList.subtitle') }}</p>
            </div>
            ...
            <div class="mb-6 flex flex-wrap items-center gap-4">
                ...
                >
                    ...
                    {{ t('flightList.searchFlight') }}
                </Link>

                <div class="flex overflow-hidden rounded-lg border border-gray-200 shadow-sm">
                    <button
                        v-for="(labelKey, k) in { alle: 'flightList.allClasses', economy: 'flight.economy', business: 'flight.business' }"
                        :key="k"
                        @click="filterOp(k)"
                        :class="[
                            'px-4 py-2 text-sm font-medium transition-colors',
                            klasse === k ? 'bg-blue-900 text-white' : 'bg-white text-gray-700 hover:bg-gray-50',
                        ]"
                    >
                        {{ t(labelKey) }}
                    </button>
            ...
            <div v-if="vluchten.length === 0" class="rounded-lg border border-gray-200 bg-white p-12 text-center text-gray-500">
                ...
                <p class="text-lg font-medium">{{ t('flightList.noFlights') }}</p>
                <p class="mt-1 text-sm">{{ t('flightList.noFlightsSub') }}</p>
            </div>
                    ...
                    <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:gap-6">
                            <div class="flex min-w-[120px] flex-col">
                                <span class="text-xs font-semibold uppercase tracking-wider text-gray-400">{{ t('flight.airline') }}</span>
                                <span class="font-semibold text-blue-900">{{ vlucht.luchtvaartmaatschappij?.naam ?? '—' }}</span>
                        ...
                        <div class="flex flex-row gap-3 sm:flex-col sm:items-end">
                            <div class="flex gap-3">
                                <div class="rounded-lg bg-blue-50 px-3 py-2 text-center">
                                    ...
                                    <p class="text-xs text-gray-500">{{ vlucht.beschikbaar_economy }} {{ t('flight.seats') }}</p>
                                </div>
                                <div class="rounded-lg bg-yellow-50 px-3 py-2 text-center">
                                    ...
                                    <p class="text-xs text-gray-500">{{ vlucht.beschikbaar_business }} {{ t('flight.seats') }}</p>
                                </div>
                            </div>
